...
Once all .nc files are downloaded, the next step is to extract climate variables conforming to desired spatial and temporal extents from them. This can be done for either a single point in space with `point_nc_to_df` or for a an area with `grid_nc_to_df` (currently not properly implemented). Both functions output a dataframe where each row is a single observation in time and space and each column is a climatic variable. Note that these functions use the same start and end times defined prior to building the request. Given that `runauto` in the `microclima` package only accepts date ranges within single years, you will need to create a data frame for each yearly block. For example, if your period of interest spans multiple years (and you have used the previous code to download multiple .nc files, run this next block of code for each year):
