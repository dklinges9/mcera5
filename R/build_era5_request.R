...
#' are required.
#' @param by_month logical indicating whether to split the request by month (default is TRUE)
...
build_era5_request <- function(xmin, xmax, ymin, ymax, start_time, end_time,
                               by_month = TRUE, outfile_name = "era5_out") {
  # input checks
  if (missing(xmin)) {
  ...
  if (by_month) {  # iterate over all focal months
    ...
    } else { # iterate over all focal years
